...
function mypost_category_posts_enqueue_stylesheet(){
	
	wp_register_style('category_list_css', plugins_url('css/style.css',__FILE__));
	wp_enqueue_style('category_list_css');
}

add_action('wp_enqueue_scripts', 'mypost_category_posts_enqueue_stylesheet');

/*******************************************************
	Fetch latest post in up to four categories and output then with theirs featured image
*******************************************************/
function mypost_category_post_listing(){
	...
	if($categories){
		...
		<section class="category-posts">
			...
			<?php echo '<h2>'. __('Latest Posts', 'mypost'). '</h2>';
...
add_action('mypost_after_content','mypost_category_post_listing');
